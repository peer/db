...
func getDumpJSONs(
	...
	if len(matches) == 1 {
		...
		downloadReader, errE := NewDownloader(config.Client, req)
		if errE != nil {
			errs <- errE
			return
		}
		defer downloadReader.Close()
		compressedSize = downloadReader.Length()
		...
		defer func() {
			...
			if err != nil || downloadReader.Length() != fileLength {
	...
	countingReader := &CountingReader{Reader: compressedReader} //nolint:exhaustivestruct
	ticker := NewTicker(ctx, countingReader, compressedSize, progressPrintRate)
	defer ticker.Stop()
	go func() {
...
func Process(config *ProcessConfig) errors.E {
