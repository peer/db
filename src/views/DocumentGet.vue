          ...
          -->
            <TabGroup v-else manual :selected-index="selectedTabIndex" @change="onTabChange">
              ...
              <TabPanels as="template">
                ...
                <TabPanel v-if="hasPermission(CAN_CHANGES_DOCUMENT)" tabindex="-1" class="pd-documentget-panel-history outline-none">
      ...
      >
        <div v-if="!siteContext.features.hideDocumentActions && (hasPermission(CAN_EDIT_DOCUMENT) || hasPermission(CAN_DELETE_DOCUMENT))" class="flex flex-col gap-2">
          <WithLock v-if="hasPermission(CAN_EDIT_DOCUMENT)" :lock="getEditLock">
            <Button id="documentget-button-edit" :progress="editBusy" type="button" class="w-full" @click.prevent="onEdit">{{ t("common.buttons.edit") }}</Button>
          </WithLock>
          <ButtonLink v-if="hasPermission(CAN_DELETE_DOCUMENT)" id="documentget-button-delete" :to="{ name: 'DocumentDelete', params: { id } }" class="w-full">{{
