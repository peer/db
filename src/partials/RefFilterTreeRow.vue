    ...
    <div class="pd-reffiltertreerow-row flex items-baseline gap-x-1">
      ...
      <template v-else>
        ...
        <template v-if="isSpecialValueId(node.res.id)">
          <label :for="inputId" class="pd-reffiltertreerow-label-special" :class="locked ? 'cursor-not-allowed text-gray-600' : 'cursor-pointer'"
            ><i>{{ specialValueLabel(node.res.id, t) }}</i></label
          >
          <label :for="inputId" class="pd-reffiltertreerow-count-special" :class="locked ? 'cursor-not-allowed text-gray-600' : 'cursor-pointer'"
