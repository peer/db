<!--
Renders a document as the contents of a search result card. The document is given: fetching it is
SearchResult's part, so a page which has the document already renders it through this component instead
of having it fetched a second time. The classes and fields the rendering needs are loaded here, because
they follow from the document rather than from who has it.
-->
...
const duplicateOfLink = computed(() => ({
  ...
  params: route.params,
  query: encodeQuery({ ...route.query, at: props.doc.id }),
}))

// The wrapper the ref is on renders no box of its own (display: contents), so it leaves the card's layout
// as it is while giving the class loading an element to attach its requests to.
...
const description = computed(() => {
  const claims = selectClaimsByLanguage(props.doc.claims, "html", effectiveDescriptionProperties.value, locale.value, (c) => c.length > 0 && !!c[0].html)
...
const previewFiles = computed<string[]>(() => {
  return getClaimsOfTypeWithConfidence(props.doc.claims, "link", previewProperties.value).map((c) => c.iri)
})
