...
watch:
	CompileDaemon -build="make --silent peerdb" -command="./peerdb -D -k localhost+2.pem -K localhost+2-key.pem -c config.yml" -include="*.json" -include="go.*" -include="config.yml" -exclude=".*" -exclude-dir=".*" -exclude-dir=output -exclude-dir=node_modules -exclude-dir=lib -exclude-dir=coverage -graceful-kill=true -log-prefix=false -color=true
