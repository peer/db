...
.PHONY: build peerdb wikipedia mapping build-static lib test test-ci lint lint-ci fmt fmt-ci upgrade clean release lint-docs lint-docs-ci audit encrypt decrypt sops watch

build: peerdb wikipedia mapping
...
clean:
	find dist -mindepth 1 ! -path "dist/dist.go" -delete
	rm -rf coverage.* codeclimate.json tests.xml coverage lib peerdb wikipedia mapping
